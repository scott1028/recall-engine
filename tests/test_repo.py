...
def remote_repo(tmp_path):
    ...
    return bare
# --- path mode ---
def test_path_mode_returns_resolved_dir_and_rejects_missing_dir(tmp_path):
...
def test_ssh_mode_clone_failure_raises(tmp_path, monkeypatch, fake_home):
    ...
    with pytest.raises(RepoError, match="git clone"):
        ensure_repo(settings)
# --- ssh key resolution ---
def test_key_detection_picks_available_key_prefers_ed25519_and_reports_missing(
    tmp_path, monkeypatch
):
    rsa_home = tmp_path / "home-rsa"
    (rsa_home / ".ssh").mkdir(parents=True)
    (rsa_home / ".ssh" / "id_rsa").write_text("dummy\n")
    monkeypatch.setenv("HOME", str(rsa_home))
    assert resolve_ssh_key(None) == rsa_home / ".ssh" / "id_rsa"
    preferred_home = tmp_path / "home-preferred"
    (preferred_home / ".ssh").mkdir(parents=True)
    (preferred_home / ".ssh" / "id_rsa").write_text("dummy\n")
    (preferred_home / ".ssh" / "id_ed25519").write_text("dummy\n")
    monkeypatch.setenv("HOME", str(preferred_home))
    assert resolve_ssh_key(None) == preferred_home / ".ssh" / "id_ed25519"
    ...
    for name in ("id_ed25519", "id_ecdsa", "id_rsa"):
        assert str(empty_home / ".ssh" / name) in str(excinfo.value)
def test_explicit_ssh_key_wins_and_missing_file_raises(tmp_path):
    key = tmp_path / "my-key"
